...
def solution(input_: str, bag_configuration: BagConfiguration, part: int = 1) -> int:
    """
    Solve the day 2 problem!
    """
    games = Games.from_file(input_)

    return games.is_possible(bag_configuration) if part == 1 else games.power()
...
def main():
    ...
    sample_1 = solution("sample.data", bag_configuration, 1)
    part_1 = solution("input.data", bag_configuration, 1)
    sample_2 = solution("sample.data", bag_configuration, 2)
    part_2 = solution("input.data", bag_configuration, 2)
