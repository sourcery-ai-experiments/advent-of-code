...
def solution(input_: str, part: int) -> int:
    """
    Solve the day 3 problem!
    """
    engine_schematic = EngineSchematic.from_file(input_)

    if part == 1:
        return sum(part.number for part in engine_schematic.part_numbers)
    else:
        return sum(engine_schematic.gear_ratios())
...
def main():
    ...
    sample_1 = solution("sample.data", 1)
    part_1 = solution("input.data", 1)
    sample_2 = solution("sample.data", 2)
    part_2 = solution("input.data", 2)
